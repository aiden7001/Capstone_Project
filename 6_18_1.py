...
for frame in camera.capture_continuous( rawCapture,format="bgr",use_video_port=True):
        ...
        if fcounter == 20:
                ...
                for row in result:                        
                        ...
                        parse = ''.join(temp)
                        print 'id = ' + parse #for check
                        print 's_ratio = ' + str(s_ratio)

                       
                        sql_4 = "select id, sum_ratio from registration_table where id=%s and sum_ratio - %s<0.15 and sum_ratio-%s>-0.15;"
                        cur.execute(sql_4,(parse,s_ratio,s_ratio))
                        result_4 = cur.fetchall()

                        for row_sub, row_sub2 in result_4:
                                print row_sub, row_sub2
